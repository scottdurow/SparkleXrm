...
SparkleXrm.Sdk.Messages.RetrieveMetadataChangesRequest.prototype = {
    ...
    _customWebApiImplementation: function SparkleXrm_Sdk_Messages_RetrieveMetadataChangesRequest$_customWebApiImplementation(request, callback, errorCallback, async) {
        ...
        SparkleXrm.Sdk.WebApiOrganizationServiceProxy._sendRequest('EntityDefinition', 'EntityDefinitions', query, 'GET', null, async, function(state) {
            ...
            var tasks = false;
            var additionalRequests = new SparkleXrm.TaskIterrator();
            var $enum1 = ss.IEnumerator.getEnumerator(response.entityMetadata);
            while ($enum1.moveNext()) {
                var entity = $enum1.current;
                var $enum2 = ss.IEnumerator.getEnumerator(entity.Attributes);
                while ($enum2.moveNext()) {
                    var attribute = $enum2.current;
                    if (attribute.AttributeType === 'Picklist') {
                        tasks = true;
                        var taskcontext = {};
                        taskcontext['entity'] = entity;
                        taskcontext['attribute'] = attribute;
                        additionalRequests.addTask(function(successCallBack, errorCallBack, taskstate) {
                            var resource = String.format('EntityDefinitions({0})/Attributes({1})/Microsoft.Dynamics.CRM.PicklistAttributeMetadata/OptionSet', (taskstate['entity']).MetadataId, (taskstate['attribute']).MetadataId);
                            SparkleXrm.Sdk.WebApiOrganizationServiceProxy._sendRequest('Attribute', resource, '$select=Options', 'GET', null, async, function(picklistState) {
                                var optionsetMetdata = taskstate['attribute'];
                                var picklistdata = SparkleXrm.Sdk.WebApiOrganizationServiceProxy._jsonParse(picklistState);
                                var picklistMetadata = optionsetMetdata;
                                picklistMetadata.OptionSet = picklistdata;
                                successCallBack();
                            }, errorCallback);
                        }, taskcontext);
            ...
            if (tasks) {
                additionalRequests.start(function() {
                    callback(response);
                }, function() {
                    errorCallback(new Error('Could not get optionset data'));
                });
            }
            else {
                callback(response);
