namespace Microsoft.Crm.Sdk.Fakes.Tests
{
    ...
    public class LocalPluginContext
    {
        ...
        internal Entity Target
        {
            get
            {
                if (_target == null && this.PluginExecutionContext.InputParameters.ContainsKey("Target"))
                {
                    var target = this.PluginExecutionContext.InputParameters["Target"];
                    if (target != null && target.GetType() == typeof(Entity))
                    {
                        _target = (Entity)target;
                    }
                    else if (target != null)
                    {
                        EntityReference reference = (EntityReference)target;
                        _target = new Entity(reference.LogicalName);
                        _target.Id = reference.Id;
        ...
        internal void Trace(string message)
        {
            ...
            else
            {
                this.TracingService.Trace(
                    "{3:hh:mm:ss.f}: {0}, Correlation Id: {1}, Initiating User: {2}",
                    message,
    ...
    public class Plugin : IPlugin
    {
        ...
        public void Execute(IServiceProvider serviceProvider)
        {
            ...
            localcontext.Trace(string.Format(CultureInfo.InvariantCulture, "Entered {0}.Execute() {1} {2} {3}", this.ChildClassName, localcontext.PluginExecutionContext.Stage, localcontext.PluginExecutionContext.MessageName, localcontext.PluginExecutionContext.PrimaryEntityName));
